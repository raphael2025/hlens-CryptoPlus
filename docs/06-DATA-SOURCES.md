...
> 回答三个问题：**我们要什么数据**（§2）、**从哪个官方接口拿**（§3）、**限速与预算怎么算**（§4）。字段名以 `api/openapi.yaml` 与 `03-DEVELOPMENT.md` §3 的表为准。限速数字标注来源：`官方` = 官方文档原文（2026-09-12 核对）、`实测` = 我们的仓库或 Actions 探测记录、`未验证` = 文档未明写，按保守值记账。
>
...
WebSocket 订阅：`allMids`、`trades{coin}`、`candle{coin,interval}`、`userFills{user}`、`userEvents`、`userFundings`、`activeAssetCtx{coin}`、`bbo{coin}`、`clearinghouseState`。官方 WS 限制：**每 IP ≤ 10 连接、每分钟 ≤ 30 次新连接、≤ 1000 订阅、≤ 10 个不同 user、≤ 2000 消息/min、≤ 100 个在飞 post**。**注意"每 IP ≤ 10 个不同 user"**：这条与 `03-DEVELOPMENT` §4.1"WS `userFills` 订阅 hot 200 钱包"冲突，见 §7。WS 不消耗 `/info` 权重（文档结构隐含，非原文）。
...
- `03-DEVELOPMENT` §4.1 的限速预算段以本文 §4 为准；§4.1 "WS `userFills` hot 200" 待 §7-1 验证后改写。
- `05-DOCKER` 硬规则 2（出口 IP）与本文 §5、§6-1 一致。
