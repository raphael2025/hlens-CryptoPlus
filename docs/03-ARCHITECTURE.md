...
- **算式方向的更正（2026-09-20，M1-C 查实）**：上一行原写 `oi_base × mark × mult`，**方向反了，照它做会把 `1000PEPE` 这类币放大 `mult²`（10⁶ 倍）**。链条是：交易所的 `openInterest` 返回**张数**（`1000PEPEUSDT` 一张 = 1000 PEPE，`mult = 1000`）；**适配器的 `to_base_units` 已经乘过一次 `mult`**，所以写进 `oi_base` 的已经是币本身的单位；而 `mark` 是**一张**的美元价。于是 `oi_usd = oi_base ÷ mult × mark`。核验：`1.25e9 PEPE × $0.0000082/PEPE = $10,250`；`÷ mult` 得 `$10,250` ✓，`× mult` 得 `$1.025e10` ✗。**教训**：这条裁决写下时只看了「两所都只发基础单位的持仓量」这句话，没有回头看适配器已经做过一次归一——**派生列的算式必须连着「它的输入是谁归一的」一起写**，否则同一次换算会被做两遍。
...
| **HL `/info` 权重** | 1200/分 | **1080/分**（90%） | **953/分 —— `实测`（M1-B，`样本不足`）** | **127/分** | **44/分** | **953 + 127 = 1080 = 天花板，但 ⚠ 见下，这里打不出 ✓** |
| HL WS | 10 连接 · 1000 订阅 · **10 个不同 user** | 8 连接 · 800 订阅 · 8 user | **0 连接 · 0 user（`实测`，M1-B）** —— 旧采集器是纯 REST，不占席位 | 8 连接 · 800 订阅 · 8 user 全部可用 | 1 订阅 | 数完了，席位不是约束 |
...
> ⚠ **M1-B 实测回填（2026-09-20）：上表 HL 那一行的「✓」被撤掉了，因为它现在会骗人。**
> 953 是旧采集器在本出口的实测 p95（它自己的按出口 IP 记账的控制面，28 样本 / 29 分钟，`样本不足`、是下界），`953 + 127 = 1080` 在算术上仍然成立。**但同一窗口的观测最大值是 1147 —— 它一家就已经单分钟越过 1080 这条天花板**；而且它给自己配的上限是 **1200 权重/分**（每出口 IP 一份），**它完全不知道本项目存在**。
> 所以：**「reserved + 我们可用 = 天花板」这个等式描述的是账本，不是出口的真实占用。** 账本里填 953 而不是 1147 或 1200，只是因为「预留额 > 天花板」会让加载器直接拒绝加载——**那个拒绝是对的**，它说的正是「这个桶没有安全的分法」。
> **占位值 960 的方向判断反了：它不是保守，是乐观。** 真正的修法只有一个，且不在本项目手里——**把旧采集器的自配额压到 1080 以内，由 raphael 决定**。在那之前，HL 侧的一切余量结论都带 `样本不足`，且 M5 的 HL 现采**仍然门控于退役**（比原文的放行条件更硬：原文说「若旧采集器实际只用掉一小部分，M5 可能根本不必等」，实测不是一小部分）。
> 证据与命令见 `docs/reports/2026-09-20-m1b-hub-inventory.md`。
...
~~**M1-B 的第一件事就是把这个占位值换成实测值**（读旧采集器**自身的账本与日志**，不要为了测它而去打交易所——那是在花我们自己要省的预算；日常 / 回补 / 高波动三档各取 p95）。~~ **已完成（2026-09-20，见 `docs/reports/2026-09-20-m1b-hub-inventory.md`）。** 全程零交易所调用；三档没测全，只拿到一档约 29 分钟的抽样，因为那台机器的控制面只暴露滑动 60 秒的当前值、历史表不可达，故结论标 `样本不足` 并按下界处理。实测回填后 `reserved.hyperliquid.hub_legacy` = **953**、我们可用 **127**，preflight 已重跑。

**M5 的门控结论随之重算，答案是「仍然门控」，而且比这里原来写的更硬。** 原文的放行条件是「若旧采集器实际只用掉一小部分，M5 可能根本不必等它退役」——**实测它用掉的不是一小部分**（p95 953/1080），且它的自配额 1200 本身就高过我们整条出口的天花板、实测单分钟已打到 1147。M5 的 200 地址 × `clearinghouseState` = +80/分 在这种共享条件下不安全。**所以「M5 必须等退役」现在是有证据的结论，不再是占位假设；同样，也仍然不要假装有 1080 权重/分可用。**
...
**HL 快道地板重算（M1-B，2026-09-20）：维持 `40`，天花板从 120 变 127 不影响它。** 上一版注记说 40 是「120 这个过渡期天花板夹出来的值」，**这句话本身要更正**：40 从来不是从天花板推出来的，它是**快道自己的用量** —— `metaAndAssetCtxs` W=20 × 每 30 秒一次 = 40 权重/分。§6.1 的算式「快道 40 + 元数据与重试 20 = 60」是**由 40 推出 60**，不是由 120 推出 40；天花板动了，这条推导的输入一个都没动。多出来的那 7 权重/分也买不到任何东西：这条道上**一次重试的粒度就是 20**（一整个 `metaAndAssetCtxs`），7 < 20，连半次重试都塞不进去。想给快道留一次重试的余量，地板要从 40 跳到 60，那会把 127 里的 47% 圈给一条稳态只用 40 的道，而它实际要防的「其余 resident 挤占」在 HL 侧只有 4 权重/分（元数据）——**没有可挤占的对象，圈更多是假保护**。真正威胁这条快道的不是内部挤占，是 §6.1 上面那条 ⚠（另一个消费者单分钟 1147），而那个问题地板解决不了。**结论：`fast_lane_floor_per_min: 40` 不改，`reserve_per_min: 60` 不改。**
...
4. **hub 的家底数字**（**M1-B 部分回答，2026-09-20**）：**HL WS 席位已数完 —— 旧采集器是纯 REST，占 0 连接 0 user，8/800/8 全部可用**，这一项结掉。**其余仍然阻塞 M1-F，且「只要开机就能测」这句不成立**：可用磁盘 `D_free` · CPU · 内存 **在生产主机上未测得**，因为跑 M1-B 的代理在那台机器没有 SSH 授权（公钥未装、Tailscale SSH 未对该路径开放），只能读它对外开的控制面 HTTP。**需要你给一次访问，或自己在那台机器上跑一遍。**（M1-B 顺带记了开发机的同类数字，但那是另一台机器，`§12` 三条门控线以生产主机的 `D_free` 为分母，**不能拿开发机的数顶替**。）· **旧采集器预计什么时候退役**：**仍然待你回答**，日志、配置、文档里没有任何日期或计划的线索，M1-B 不猜（它决定 M5 的 HL 现采能不能开，见第 6.1 节与第 15 节——按实测，这一条现在是硬门控）。
