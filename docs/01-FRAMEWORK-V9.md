...
## 25. raphael 的实际做法（2026-09-25 补充）

- **4H**：看 EMA 9 / 21 / 55 / 200 的回踩与结构。
- **1H**：看维加斯通道的回踩与结构；只用过滤线 EMA12 与 A 组 EMA144 / 169，B 组（576 / 676）基本不看。
- **15m**：看关键位置的止损是否已被打掉，打完之后是否反转。
- 震荡行情不做；关键位置没有出现猎杀，不进场。
- 自评：入场好，缺的是好的止盈与止损。

## 26. raphael 的出场与抄底思路（2026-09-25 补充）

- 开仓后下跌：多头趋势里判断是诱空还是回调；关键位置收出下影线（跌下去又收回），就认为没事。
- 多头趋势里的快速下跌，敢在关键位置抄底。
- 止盈止损应该是动态的、根据市场情况，而不是固定距离；**该固定的固定，该动态的动态**。
- 上涨趋势里的回调，可以先平掉再找机会，而不是扛深度回调。
