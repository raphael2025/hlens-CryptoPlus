...
- Read `docs/00-PROJECT.md` (§4 principles, §7 architecture/seams/stack table, §8 milestones), `docs/01-FEATURES.md` (feature definitions), `docs/02-ARCHITECTURE.md` (modules, data model, processes, export, build order), `docs/06-DATA-SOURCES.md` (data, endpoints, rate limits), and the adapter protocol in code — `packages/hlens-core/src/hlens_core/adapters/base.py` (protocol and capability declaration), `adapters/binance.py` (reference implementation) — before writing code. Field names follow the contracts in `packages/hlens-core` until an API contract exists again in M3.
